module CamaleonCms::Frontend::NavMenuHelper
  ...
  def draw_menu(args = {})
    ...
    nav_menu = current_site.nav_menus.includes(
      :metas, :term_relationships, 
      children: [:metas, :term_relationships, 
        children: [:metas, :term_relationships, 
          children: [:metas, :term_relationships, 
            children: [:metas, :term_relationships, 
              children: [:metas, :term_relationships, 
  ...
  def cama_menu_draw_items(args, nav_menu, level = 0)
    ...
    nav_menu.each do |nav_menu_item|
      _args = args.dup
