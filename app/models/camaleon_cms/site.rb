class CamaleonCms::Site < CamaleonCms::TermTaxonomy
	...
	def upload_directory(inner_directory = nil)
		# in order to create a folder structure that reflects the upload date of the file
		# a change was made on line 149. Changed the last condition in the ternary operator - replaced self.id with a date string
		date_string = Time.now.strftime('%Y/%m/%d')
		File.join(Rails.public_path, "/media/#{PluginRoutes.static_system_info["media_slug_folder"] ? self.slug : date_string}", inner_directory.to_s)
	end
