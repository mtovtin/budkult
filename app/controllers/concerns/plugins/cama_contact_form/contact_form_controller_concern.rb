  ...
  def perform_save_form(form, fields, success, errors)
    ...
    if validate_to_save_form(form, fields, errors)
      form.fields.each do |f|
        if f[:field_type] == 'file' && !fields[f[:cid]].nil?
          file_paths = []
          fields[f[:cid].to_sym].each do |file|
            ...
            res = cama_tmp_upload(file, {
              ...
              path: Rails.public_path.join("contact_form", date_string),
      ...
      if form_new.save
        ...
        cama_send_email(form.the_settings[:railscf_mail][:to], "#{form.the_settings[:railscf_mail][:subject].to_s.translate.cama_replace_codes(fields)} ##{72400 + form_new.id}", {attachments: attachments, content: content, extra_data: {fields: fields_data}})
        ...
        if form.the_settings[:railscf_mail][:to_answer].present? && (answer_to = fields[form.the_settings[:railscf_mail][:to_answer].gsub(/(\[|\])/, '').to_sym]).present?
         content = render_to_string(partial: plugin_view('contact_form/email_content'), layout: false, formats: ['html'], locals: {file_attachments: attachments, fields: fields_data, values: fields, message_body: message_body, form: form})
         cama_send_email(answer_to, "#{form.the_settings[:railscf_mail][:subject_answer].to_s.translate.cama_replace_codes(fields)} ##{72400 + form_new.id}", {content: content})
       end
     else
