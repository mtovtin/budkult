...
module Plugins::ExportContent::ExportContentHelper
  ...
  def export_content_app_before_load
    ...
    CamaleonCms::Site.class_eval do
      ...
      def custom_field_all
        CamaleonCms::CustomField.where(object_class: '_fields').includes(:custom_field_group).where(parent_id: self.custom_field_groups.pluck(:id))
      end
    end

    CamaleonCms::CustomFieldGroup.class_eval do
      def post_type
    ...
    CamaleonCms::Category.class_eval do
      def children_all
        children.map do |c|
          cc = c.attributes
    ...
    CamaleonCms::NavMenuItem.class_eval do
      def children_all
        children.map do |c|
          cc = c.attributes
          cc[:metas] = c.metas.to_a
      ...
      def post_type
        case object_class
          when "PostType_Post","PostType_Category","PostType_PostTag"
            CamaleonCms::PostType.find(objectid)
        end
      end
    end
  end
        ...
        if options[:post_types].present? || (options[:post_type_ids].present? && options[:post_type_ids].include?(pt[:id].to_s))
          ...
          if post_type.save
            ...
            if pt[:posts].present?
              ...
              #wars << "Not imported posts for post type: #{post_type.name}"
            end
    ...
    if data[:nav_menus].present?
        ...
        data[:nav_menus].each do |nav_m_data|
          if options[:nav_menus].present? || (options[:nav_menu_ids].present? && options[:nav_menu_ids].to_i.include?(nav_m_data[:id].to_i))
            ...
            nav_menu = current_site.nav_menus.where(slug: c[:slug]).first_or_create(c.permit(:name, :slug, :description, :term_order, :count, :status))
            #if nav_menu.valid?
              save_metas_data(nav_menu, nav_m_data[:metas]) if nav_m_data[:metas].present?
              #save_field_group(nav_menu, nav_m_data[:get_field_groups]) if nav_m_data[:get_field_groups].present?
              #save_field_values(nav_menu, nav_m_data[:field_values])
  ...
  def _parse_codes(text)
    text.gsub("{media_url}", "#{cama_root_url}media/#{current_site.id}")
        .gsub("{theme_url}", "#{cama_root_url}assets/themes/#{current_site.get_theme_slug}/assets/")
        .gsub("{themes_url}", "#{cama_root_url}assets/themes/")
        .gsub("{root_url}", "#{cama_root_url}")
  end

  # convert current urls into url codes
  def _parse_codes2(text)
    text.gsub("#{cama_root_url}media/#{current_site.id}", "{media_url}")
               .gsub("#{cama_root_url}", "{root_url}")
  end
  ...
  def save_children_nav_menu(parent, children_data = [])
    ...
    if children_data.present?
      children_data.each do |nav_item_data|
        ...
        nav_item = parent.children.new(c.permit(:name, :slug, :description, :term_order, :status))
        nav_item.save!
        url = nav_item.url.to_s.to_i
        kind = nav_item.kind.to_s.to_sym
        nav_item.update(url: @_cache_ids_for_nav_menu[kind][url]) if @_cache_ids_for_nav_menu[kind].present? && @_cache_ids_for_nav_menu[kind][url].present?
        save_field_values(nav_item, nav_item_data[:field_values])
  ...
  def save_field_values(object, values)
    (values || []).each do |fv|
      object.set_field_value(fv[:custom_field_slug], fv[:value], {order: fv[:term_order], group_number: fv[:group_number], clear: false})
    end
  end
end
