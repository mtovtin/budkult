...
=end
class Plugins::ExportContent::AdminController < CamaleonCms::Apps::PluginsAdminController
  def settings
  ...
  def export
    ...
    obj[:plugins] = {}
    obj[:nav_menus] = JSON.parse(current_site.nav_menus.to_json(:include => [{:children => {:methods => [:children_all], :include => [:metas, :field_values]}}, :metas, obj_group, :field_values])) if params[:data][:nav_menus].present?
  ...
  def import
    case params[:method]
      ...
      when 'preview'
        file = cama_tmp_upload(Rails.public_path.to_s + params[:url])
