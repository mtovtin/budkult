...
module SimpleCalendar
  ...
  def start_date
    if view_context.params.has_key?(:year) && view_context.params.has_key?(:month)
      Date.parse("#{view_context.params.fetch(:year)}-#{view_context.params.fetch(:month)}-01}").to_date
    else
